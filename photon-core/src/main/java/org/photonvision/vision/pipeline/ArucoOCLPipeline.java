...
import org.photonvision.vision.frame.UFrame;
import org.photonvision.vision.pipe.CVPipe.CVPipeResult;
import org.photonvision.vision.pipe.impl.ArucoDetectionPipeParams;
import org.photonvision.vision.pipe.impl.ArucoOCLDetectionPipe;
import org.photonvision.vision.pipe.impl.ArucoPoseEstimatorPipe;
...
import org.photonvision.vision.pipeline.result.CVUPipelineResult;
...
public class ArucoOCLPipeline extends CVUPipeline<CVUPipelineResult, ArucoPipelineSettings> {
    private ArucoOCLDetectionPipe arucoDetectionPipe = new ArucoOCLDetectionPipe();
    private ArucoPoseEstimatorPipe singleTagPoseEstimatorPipe = new ArucoPoseEstimatorPipe();
    private final MultiTargetPNPPipe multiTagPNPPipe = new MultiTargetPNPPipe();
    private final CalculateFPSPipe calculateFPSPipe = new CalculateFPSPipe();

    public ArucoOCLPipeline() {
        super(FrameThresholdType.GREYSCALE);
        settings = new ArucoPipelineSettings();
    }

    public ArucoOCLPipeline(ArucoPipelineSettings settings) {
        super(FrameThresholdType.GREYSCALE);
    ...
    @Override
    protected CVUPipelineResult process(UFrame frame, ArucoPipelineSettings settings) {
        long sumPipeNanosElapsed = 0L;

        if (frame.type != FrameThresholdType.GREYSCALE) {
            // We asked for a GREYSCALE frame, but didn't get one -- best we can do is give up
            return new CVUPipelineResult(frame.sequenceID, 0, 0, List.of(), frame);
        }
        ...
        if (settings.debugThreshold) {
            drawThresholdFrame(
                    frame.processedImage.getUMat(),
                    frame.colorImage.getUMat(),
        ...
        return new CVUPipelineResult(
                frame.sequenceID, sumPipeNanosElapsed, fps, targetList, multiTagResult, frame);
    }

    private void drawThresholdFrame(UMat greyMat, UMat outputMat, int windowSize, double constant) {
